...
def main(force: bool = False):
    ...
    logger.info(f"Saved {BASELINE_MEAN_ERRORS_PARQUET} with index=False to avoid duplicate index")
    merged_df.to_csv(BASELINE_MEAN_ERRORS_CSV, index=False)
    logger.info(f"Saved {BASELINE_MEAN_ERRORS_CSV} with index=False to avoid duplicate index")
