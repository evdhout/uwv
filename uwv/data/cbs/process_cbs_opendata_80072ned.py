...
def process_cbs_opendata_80072ned(overwrite: bool = False):
    ...
    sbi: pd.DataFrame = pd.read_csv(external_data_dir / f'{CBS80072NED}_BedrijfskenmerkenSBI2008.csv', sep=',')
    cat_groups: pd.DataFrame = pd.read_csv(external_data_dir / f'{CBS80072NED}_CategoryGroups.csv', sep=',')
    ...
    logger.info("Merge sick_leave_percentage with periods and keep period status and description and calculated fields")
    sick_leave_percentage: pd.DataFrame = pd.read_csv(external_data_dir / f'{CBS80072NED}_UntypedDataSet.csv',
                                                      sep=',',
                                                      na_values='       .'
                                                      )
