...
from uwv.config import KNMI_AVG_TEMP_MONTH_URL, KNMI_AVG_TEMP, KNMI_INTERIM_DATA_DIR, KNMI_EXTERNAL_DATA_DIR, KNMI_PROCESSED_DATA_DIR
    ...
    with open(KNMI_EXTERNAL_DATA_DIR / KNMI_AVG_TEMP, "r") as knmi_file:
        ...
        with open(KNMI_INTERIM_DATA_DIR / KNMI_AVG_TEMP, "w") as knmi_interim_file:
            knmi_interim_file.writelines(
                [line.replace(' ', '') 
                 for line 
                 in filter(lambda line: line[:4] in ["260,", "STN," ], knmi_file)]
            )

def get_knmi_avg_monthly_temp() -> pd.DataFrame:
    df = pd.read_csv(KNMI_INTERIM_DATA_DIR / KNMI_AVG_TEMP,
                       dtype="Int64",
                       na_values=["      "])
...
if __name__ == "__main__":
    ...
    pd_knmi_long = pd_knmi.melt(id_vars="period_year", 
