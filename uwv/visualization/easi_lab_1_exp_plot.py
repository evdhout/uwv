...
def get_data(period: str = 'JJ', max_year: int = 2022) -> pd.DataFrame:
    ...
    """
    selection_path = CBS_OPENDATA_PROCESSED_DATA_DIR / f"{CBS80072NED}_{period}_lte{max_year}.parquet"
    if selection_path.exists():
        return pd.read_parquet(selection_path)
    else:
        sbi_with_parents_path = CBS_OPENDATA_PROCESSED_DATA_DIR / f"{CBS80072NED}_with_parents.parquet"
        if sbi_with_parents_path.exists():
            ...
            sbi_hierarchy = pd.read_csv(RAW_DATA_DIR / "sbi80072ned_hierarchy.csv", dtype={'Key': 'str'})[
            ...
            slp_full.loc[slp_full.sbi == '301000', 'parenttitle'] = 'A Landbouw, bosbouw en visserij'
    ...
    slp_selection: pd.DataFrame = slp_full[(slp_full.period_type == period)
...
def easi_lab_1_exp_plot():
    ...
    for q in [1, 2, 3, 4]:
        fig2 = px.box(slp_quarter[slp_quarter.period_quarter_number == q],
                      x="sick_leave_percentage",
                      y="sbi_title",
                      orientation="h",
                      color='parenttitle',
